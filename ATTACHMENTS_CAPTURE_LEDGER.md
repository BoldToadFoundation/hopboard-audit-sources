...
| GA | — ✅ **UNBLOCKED 2026-09-06** — the "403s ALL UAs" finding was a probe artifact: a fetch-API request 403s, a real page navigation does not. Both target PDFs download under headless Playwright (C-200 195,543 B, C-100 1,071,792 B, `%PDF-` magic). Not a Tony-browser item. WAS: sos.ga.gov 403s ALL UAs from this machine (the GA seven-source wall). Target: How-To Guide Charities page / renewal-notice PDF | — | YES per search snippets — **NOT fetch-confirmed** | — | `required_attachments_renewal`: 6 items. **Needs Tony's-browser path or alternate egress; the ONE open catch-up item** |
